...
async function run() {
    ...
    // db.serialize is only available on the SQLite object, but db.js exports a wrapper in some cases
    // Checking if db has serialize
    if (!db.serialize) {
        console.log("⚠️ db.serialize missing (wrapper setup?). Using db.all directly.");
    }
